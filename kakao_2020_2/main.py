...
def commands_random(truck_location_id,location_id):
    commands=[]
...
for i in range(720):
    ...
    for location in locations:
        ...
        if location["located_bikes_count"] <= 1:
            truck_id = get_truck_full(trucks,location["id"],1)
            if truck_id != -1:
        ...
        if location["located_bikes_count"] >=2:
            ...
            if truck_id != -1:
                if truck_id not in done:
                    ...
                    continue
    for truck in trucks:
        #truck_commands =commands_random(truck["id"],int(truck["id"])//5+2)
        truck_commands =commands_random(truck["id"],12)
